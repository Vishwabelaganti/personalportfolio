...
export class StudyScene {
 ...
 buildWorld(){
  ...
  this.mesh(new THREE.BoxGeometry(9.8,.28,.42),this.darkWood,0,4.65,-.4);
  for(let i=0;i<19;i++)this.mesh(new THREE.BoxGeometry(.12,.1,1.2),this.darkWood,-4.5+i*.5,4.8,-.6);
  ...
  [-3.0,3.0].forEach(x=>{this.mesh(new THREE.CylinderGeometry(.012,.012,.7,6),this.darkWood,x,4.25,-.4);const mat=new THREE.MeshStandardMaterial({color:0xffdeb2,emissive:0xffbd78,emissiveIntensity:1.2,roughness:.9});this.mesh(new THREE.SphereGeometry(.36,24,18),mat,x,3.64,-.4).scale.y=1.25;for(let j=0;j<6;j++){const ring=this.mesh(new THREE.TorusGeometry(.33*Math.sin((j+1)/7*Math.PI),.01,4,24),this.darkWood,x,3.21+j*.16,-.4);ring.rotation.x=Math.PI/2;}this.mesh(new THREE.CylinderGeometry(.12,.12,.08,16),this.darkWood,x,3.2,-.4);const light=new THREE.PointLight(0xffcc8a,5,8);light.position.set(x,3.64,-.1);this.scene.add(light);this.lights.push({light,material:mat});});
 ...
 frame(time){this.raf=requestAnimationFrame(this.frame);const dt=Math.min((time-(this.last||time))/1000,.05);this.last=time;if(document.hidden)return;if(this.motion)this.time+=dt;this.chimes.forEach(c=>{c.energy=Math.max(0,c.energy-dt*.5);c.pivot.rotation.z=this.motion?Math.sin(this.time*2.4+c.phase)*(this.wind*.065+c.energy*.28):0;c.pivot.rotation.x=this.motion?Math.sin(this.time*1.7+c.phase)*this.wind*.035:0;});this.sail.rotation.z=Math.sin(this.time*1.7)*this.wind*.16;this.rainPoints.visible=this.rain>.01;if(this.rainPoints.visible&&this.motion){const a=this.rainPoints.geometry.attributes.position;for(let i=0;i<a.count;i++){let y=a.getY(i)-dt*(4+this.rain*4);if(y<-.4)y=10;a.setY(i,y);}a.needsUpdate=true;this.rainPoints.material.opacity=this.rain*.75;}this.renderer.render(this.scene,this.camera);}
 dispose(){cancelAnimationFrame(this.raf);this.resizeObserver.disconnect();this.container.removeEventListener('pointerdown',this.pick);this.scene.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});this.renderer.dispose();}
}
