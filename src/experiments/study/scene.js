...
export class StudyScene {
 constructor(container,onChime){
  ...
  this.scene.add(this.camera);this.camera.add(this.backdrop);this.backdrop.position.set(0,0,-40);this.backdrop.material.fog=false;
  ...
  this.lastStrikes=new Map();
  this.pick=e=>{const r=container.getBoundingClientRect();pointer.set((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1);raycaster.setFromCamera(pointer,this.camera);const hit=raycaster.intersectObjects(this.hitMeshes)[0];if(hit){const note=hit.object.userData.note,now=performance.now();if(now-(this.lastStrikes.get(note)||-1000)>160){this.lastStrikes.set(note,now);this.onChime(note);}}};
  this.down=e=>{this.dragging=true;container.setPointerCapture(e.pointerId);this.pick(e);};this.move=e=>{if(this.dragging){this.pick(e);this.gust=Math.min(1,(this.gust||0)+Math.abs(e.movementX||0)*.008);}};this.up=()=>{this.dragging=false;};
  container.addEventListener('pointerdown',this.down);container.addEventListener('pointermove',this.move);container.addEventListener('pointerup',this.up);container.addEventListener('pointercancel',this.up);container.addEventListener('lostpointercapture',this.up);this.frame=this.frame.bind(this);this.raf=requestAnimationFrame(this.frame);
 }
 ...
 buildWorld(){
  ...
  this.floor=this.mesh(new THREE.PlaneGeometry(60,16),this.floorWood,0,0,1.1);this.floor.rotation.x=-Math.PI/2;this.woodTexture.repeat.set(7.3,2);
  ...
  this.mesh(new THREE.CylinderGeometry(1.35,1.4,.13,48),this.darkWood,0,4.25,.9).scale.z=.28;
  this.mesh(new THREE.CylinderGeometry(1.38,1.38,.025,48),this.brass,0,4.3,.9).scale.z=.28;
  ...
  const ceramic=new THREE.MeshPhysicalMaterial({color:0xc1d2c8,metalness:.12,roughness:.27,clearcoat:.8});
  for(let i=0;i<5;i++){const pivot=new THREE.Group();pivot.position.set((i-2)*.48,4.18,.9);this.scene.add(pivot);const length=1.12+(4-i)*.13;this.mesh(new THREE.CylinderGeometry(.009,.009,.48,8),this.darkWood,0,-.24,0,pivot);const tube=this.mesh(new THREE.CylinderGeometry(.09,.1,length,32,1,true),ceramic,0,-.48-length/2,0,pivot);tube.userData.note=i;this.hitMeshes.push(tube);for(const y of [-.49,-.59,-.48-length+.1,-.48-length]){const ring=this.mesh(new THREE.TorusGeometry(.096,.012,8,32),this.brass,0,y,0,pivot);ring.rotation.x=Math.PI/2;}const bead=this.mesh(new THREE.SphereGeometry(.048,12,8),this.brass,0,-.32,0,pivot);this.chimes.push({pivot,energy:0,phase:i*.75});}
  this.mesh(new THREE.SphereGeometry(.11,24,12),this.darkWood,0,2.33,1.07);const leaf=new THREE.Shape();leaf.moveTo(0,.25);leaf.bezierCurveTo(.23,.12,.2,-.14,0,-.28);leaf.bezierCurveTo(-.14,-.06,-.19,.12,0,.25);this.sail=this.mesh(new THREE.ExtrudeGeometry(leaf,{depth:.015,bevelEnabled:true,bevelSize:.006,bevelThickness:.005,bevelSegments:2,steps:1}),ceramic,0,1.8,1.07);this.mesh(new THREE.CylinderGeometry(.007,.007,.65,8),this.darkWood,0,2.03,1.07);
 ...
 async installModels(){
  ...
  const put=(name,options,sway=false)=>{if(!models[name])return;const group=placeModel(models[name],options);this.scene.add(group);if(sway){const bounds=new THREE.Box3().setFromObject(group),pivot=new THREE.Group();pivot.position.set(options.at[0],bounds.max.y,options.at[2]);this.scene.add(pivot);pivot.attach(group);this.sway.push({group:pivot,phase:this.sway.length*.9});}return group;};
 ...
 resize(){const w=this.container.clientWidth,h=this.container.clientHeight;this.renderer.setSize(w,h);this.camera.aspect=w/Math.max(h,1);this.camera.updateProjectionMatrix();const viewHeight=2*Math.tan(THREE.MathUtils.degToRad(this.camera.fov/2))*40;const scale=Math.max(viewHeight/12.3,viewHeight*this.camera.aspect/22)*1.01;this.backdrop.scale.setScalar(scale);}
