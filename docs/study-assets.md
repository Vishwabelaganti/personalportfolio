...
The soundscape includes recorded rain and fireplace, plus synthesized breeze, stream, ocean wash, brown noise, and pink noise. The former lofi recording control is removed. The composition desk supports four phrase recipes, an eight-bar arrangement, six instrument lanes, sixteen steps per bar, chord changes, melody pitch selection, copying and clearing bars, and a blank canvas. Fresh songs vary chord order, phrase selection, melody, and drum rhythms. Playback, local save/recall, and WAV export use the same edited arrangement.
